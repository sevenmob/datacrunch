...
function start_master() {
    ...
    if [ "$DEBUG" -gt 0 ]; then
        echo sudo docker run -d -p 8080 -p 50070 --dns $NAMESERVER_IP -h master${DOMAINNAME} $VOLUME_MAP $1:$2
    fi
    MASTER=$(sudo docker run -d -p 8080 -p 50070 --dns $NAMESERVER_IP -h master${DOMAINNAME} $VOLUME_MAP $1:$2)
