...
function configure_mesos() {
    ...
    echo "export MESOS_NATIVE_LIBRARY=/opt/mesos/lib/libmesos-0.18.2.so" >> /opt/spark-$SPARK_VERSION/conf/spark-env.sh
    echo "export JAVA_LIBRARY_PATH=/opt/mesos/lib/libmesos-0.18.2.so" >> /opt/spark-$SPARK_VERSION/conf/spark-env.sh
}
